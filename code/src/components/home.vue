<template>
  <div class="page">
    ...
    <h3>
      <span @click="drawer = true" class="el-icon-more caidan"></span>
      <span class="user-star">
        {{userjson.star}}
        <span class="el-icon-star-on"></span>
      </span>
    </h3>
    <h3 style="width:90vw;margin-left:10vw;">Today's Tasks</h3>
    <el-dialog :visible.sync="drawer" fullscreen>
      <div class="caidan">
        <p @click="routerPage('/task')">
          <span class="el-icon-s-opportunity"></span> All Task
        </p>
        <p @click="routerPage('/pet')" style="background-color: aliceblue;">
          <span class="el-icon-s-opportunity"></span>Pet
        </p>
        <p @click="routerPage('/drag')">
          <span class="el-icon-s-opportunity"></span>Dragon Collection
        </p>
        <p @click="routerPage('/shop')" style="background-color: aliceblue;">
          <span class="el-icon-s-opportunity"></span>Shop
        </p>
      </div>
    </el-dialog>
    ...
    getjson() {
      this.$axios.get(`http://zhangpengfan.xyz/todo/getjson?name=${this.addData.name}`).then(res => {
        ...
        for (let i = 0; i < todo.length; i++) {
          let todoYear = new Date(todo[i].endTime).getFullYear();
          let todoMonth = new Date(todo[i].endTime).getMonth();
          let todoDay = new Date(todo[i].endTime).getDate();
          if (year == todoYear && month == todoMonth && day == todoDay) {
            this.list.push(todo[i]);
          }
        }
      });
    },
